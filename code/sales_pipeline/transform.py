...
def summarize_by_item(cleaned_data: list[dict]) -> list[dict]:
    ...
    for row in cleaned_data:
        ...
        if item not in item_totals:
            item_totals[item] = {'item': item, 'units_sold': 0, 'revenue': 0.0}
        item_totals[item]['units_sold'] += row['qty']
        item_totals[item]['revenue'] += row['total_revenue']

    return sorted(item_totals.values(), key=lambda entry: (-entry["revenue"], entry["item"]))
